...
with open(os.path.join(path, "pt_artifacts/pt_approval.teal")) as f:
    approval_source = f.read()

with open(os.path.join(path, "pt_artifacts/pt_clear.teal")) as f:
    clear_source = f.read()
...
with open(os.path.join(path, "pt_artifacts/pt_contract.json")) as f:
    js = f.read()
...
comp.add_method_call(
    app_id, c.get_method_by_name("add"), addr, sp, signer, method_args=[2, 2]
)
